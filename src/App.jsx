...
const Dashboard = ({data,setData,isMobile,onNavigate}) => {
  ...
  return (
    <div>
      {/* ── Sticky CaptureBar ── */}
      <DashCaptureBar onCapture={handleCapture}/>

      {/* ── KPI strip ── */}
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:16}}>
        {[
          {label:'Hoy',     val:`${doneCount}/${todayTasks.length}`,  color:T.accent,  icon:'✅', view:'projects'},
          {label:'Inbox',   val:inboxItems.length,                     color:T.orange,  icon:'📥', view:'inbox'},
          {label:'Hábitos', val:`${habitsDone}/${dailyHabits.length}`, color:T.purple,  icon:'🔥', view:'habits'},
        ].map(k=>(
          <div key={k.label} onClick={()=>onNavigate&&onNavigate(k.view,null)}
            style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:14,padding:'10px 10px 8px',textAlign:'center',cursor:'pointer'}}>
      ...
      {/* ── Tasks ── */}
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:18,padding:14,marginBottom:16}}>
        <div style={{display:'flex',alignItems:'center',marginBottom:12}}>
          ...
          <button onClick={()=>setAddingTask(v=>!v)}
            style={{width:22,height:22,borderRadius:8,background:`${T.accent}20`,color:T.accent,border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16,fontFamily:'inherit'}}>
            +
          </button>
        </div>
        {addingTask&&(
          ...
          {sortedTasks.map((task,i)=>{
            ...
            return (
              <div key={task.id} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'8px 0',borderBottom:i<sortedTasks.length-1?`1px solid ${T.border}`:'none',
                ...
                <div style={{flex:1,minWidth:0}}>
                  <p style={{margin:0,fontSize:12,color:task.status==='done'?T.dim:T.text,textDecoration:task.status==='done'?'line-through':'none',lineHeight:1.35}}>
                    {task.title}
                  </p>
                ...
                <button onClick={()=>deleteTask(task.id)} className="db-del-btn"
                  style={{background:'none',border:'none',padding:3,cursor:'pointer',color:T.dim,display:'flex',borderRadius:6,opacity:0,transition:'opacity .15s',fontFamily:'inherit'}}>
                  ✕
                </button>
              </div>
            );
        ...
        <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:18,padding:14,marginBottom:16}}>
          <div style={{display:'flex',alignItems:'center',marginBottom:12}}>
            ...
            <button onClick={()=>onNavigate&&onNavigate('habits',null)}
              style={{marginLeft:8,background:'none',border:`1px solid ${T.border}`,borderRadius:8,padding:'2px 8px',cursor:'pointer',color:T.dim,fontSize:10,fontFamily:'inherit'}}>
              Ver →
            </button>
          </div>
            ...
            {dailyHabits.slice(0,8).map((hb,idx)=>{
              ...
              return (
                <button key={hb.id} onClick={()=>toggleHabit(hb.id)}
                  style={{borderRadius:16,padding:'10px 6px 8px',display:'flex',flexDirection:'column',alignItems:'center',gap:3,
      ...
      {/* ── Inbox ── */}
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:18,padding:14}}>
        <div style={{display:'flex',alignItems:'center',marginBottom:12}}>
          <span style={{fontSize:12,marginRight:6}}>📥</span>
      ...
      {/* ── Pilares (acceso rápido, al final para no competir con la captura) ── */}
